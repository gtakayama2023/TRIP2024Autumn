...
void ridf2root(int runN=48, TString runname="56Co", bool tree_output=0, int loadEvt = 10000 /* 0 = NO, 1 = Yes */){
  ...
  */
  ...
  hraw_PLA_T1[0][0]= new TH1D("hraw_PLA_T_00",   Form("F3Pla-L T | run%04d;T(ns)" ,runN),600,-15000,5000);
  hraw_PLA_T1[0][1]= new TH1D("hraw_PLA_T_01",   Form("F3Pla-R T | run%04d;T(ns)" ,runN),600,-15000,5000);
  hraw_PLA_T1[1][0]= new TH1D("hraw_PLA_T_10",   Form("F5Pla-L T | run%04d;T(ns)" ,runN),600,-15000,5000);
  hraw_PLA_T1[1][1]= new TH1D("hraw_PLA_T_11",   Form("F5Pla-R T | run%04d;T(ns)" ,runN),600,-15000,5000);
  hraw_PLA_T1[2][0]= new TH1D("hraw_PLA_T_20",   Form("F7Pla-L T | run%04d;T(ns)" ,runN),600,-15000,5000);
  hraw_PLA_T1[2][1]= new TH1D("hraw_PLA_T_21",   Form("F7Pla-R T | run%04d;T(ns)" ,runN),600,-15000,5000);
  TH2D *hraw_PLA_QTC;
  ...
  hraw_Ge_Eraw[1] = new TH2D("hraw_Ge2_Eraw_F07",Form("F07Ge2 Eraw | run%04d;ID;Eraw (ch)",runN),8,0,8, 8000,-200,8400);
  TH2D *hcalib_Ge_E[2];
  hcalib_Ge_E[0] = new TH2D("hcalib_Ge_E_F07",Form("F07Ge E | run%04d;ID;E (keV)",runN),8,0,8, 4200,0,4200);
  hcalib_Ge_E[1] = new TH2D("hcalib_Ge2_E_F07",Form("F07Ge2 E | run%04d;ID;E (keV)",runN),8,0,8, 4200,0,4200);
  TH2D *hcalib_Ge_Esum[1];
  hcalib_Ge_Esum[0] = new TH2D("hcalib_Ge_Esum_F07",Form("F07Ge Esum | run%04d;ID;E (keV)",runN),2,0,2, 4200,0,4200);
  TH2D *hcalib_Ge2_Esum[1];
  hcalib_Ge2_Esum[0] = new TH2D("hcalib_Ge2_Esum_F07",Form("F07Ge2 Esum | run%04d;ID;E (keV)",runN),2,0,2, 4200,0,4200);
  TH2D *hraw_Ge_Traw[1];
  hraw_Ge_Traw[0] = new TH2D("hraw_Ge_Traw_F07",Form("F07Ge Traw | run%04d;ID;Traw (ch)",runN),8,0,8,5000,-130e3,130e3);
  TH2D *hcalib_Ge_T[1];
  hcalib_Ge_T[0] = new TH2D("hcalib_Ge_T_F07",Form("F07Ge T | run%04d;ID;T (ns)",runN),17,0,17, 2000,-2000,2000);

  TH2D *hcalib_Ge_EvsT[16];
  for(int n=0;n<8;n++){
    hcalib_Ge_EvsT[n]   = new TH2D(Form("hcalib_Ge_EvsT(User)_F07_%02d",n),Form("F07Ge E vs T(User) | run%04d;Ge User T (ns);Ge E (keV)", runN),2000,-2000,2000,4200,0,4200);
    hcalib_Ge_EvsT[n+8] = new TH2D(Form("hcalib_Ge_EvsT(BigEIPS)_F07_%02d",n),Form("F07Ge E vs T(BigRIPS) | run%04d;Ge BigRIPS T (ns);Ge E (keV)",runN),2000,-2000,2000,4200,0,4200);
  }
  ...
  //===== PID_calib3 ==========
  TH2D *hcalib_PID3[3];
  hcalib_PID3[0] = new TH2D("hcalib_PID3_0",Form("PID calib2 (AoQ35_3 vs Z3) | run%04d;AoQ35_2;Z3_2",runN),1e3,2.0,2.6,1e3,0,50); 
  hcalib_PID3[1] = new TH2D("hcalib_PID3_1",Form("PID calib2 (AoQ57_3 vs Z5) | run%04d;AoQ57_2;Z5_2",runN),1e3,1.0,2.6,1e3,0,50); 
  ...
  */
  ...
  // F7Ge 1D spectra
  TH1D *hcalib_F7Ge_ESpec[8];
  for(int ii=0;ii<4;ii++){
    hcalib_F7Ge_ESpec[ii  ] = new TH1D(Form("hcalib_F7Ge_Epec_%01d",ii  ),Form("F7Ge_L%01d_spectrum",ii+1),4000,0,4000);
    hcalib_F7Ge_ESpec[ii+4] = new TH1D(Form("hcalib_F7Ge_Epec_%01d",ii+4),Form("F7Ge_R%01d_spectrum",ii+1),4000,0,4000);
  }
  ...
  //    while(estore->GetNextEvent()&&neve<10000){
    ...
    */
    ...
    for(int m=0;m<8;m++)hcalib_F7Ge_ESpec[m]->Fill(F7Ge_E[m]);
    ...
    // ===== F7Ge ==========================================
    for(int n=0;n<8;n++) hcalib_Ge_E[0]    ->Fill(n, F7Ge_E[n]);
    for(int n=0;n<8;n++) hcalib_Ge_E[1]    ->Fill(n,F7Ge2_E[n]);
    for(int n=0;n<2;n++) hcalib_Ge_Esum[0] ->Fill(n,F7Ge_Esum[n]);
    for(int n=0;n<2;n++) hcalib_Ge2_Esum[0]->Fill(n,F7Ge2_Esum[n]);
    for(int n=0;n<17;n++)for(int m=0;m<N_Mhit;m++)if(m<v1190num[3][n])  hcalib_Ge_T[0]     ->Fill(n,F7Ge_T[n][m]);
    for(int n=0;n<8;n++) for(int m=0;m<N_Mhit;m++)if(m<v1190num[3][n])  hcalib_Ge_EvsT[n]  ->Fill(F7Ge_T[n][m],F7Ge_E[n]);
    for(int n=0;n<8;n++) for(int m=0;m<N_Mhit;m++)if(m<v1190num[3][n+8])hcalib_Ge_EvsT[n+8]->Fill(F7Ge_T[n+8][m],F7Ge2_E[n]);
  ...
  // ====== PIDcalib #3 canvas ===================================================
  TCanvas *cPIDcalib3 = new TCanvas("cPIDcalib3","cPIDcalib3",1200,1200*0.7);
  cPIDcalib3->Divide(2,1);
  cPIDcalib3->cd(1);hcalib_PID3[0]->Draw("colz");
  cPIDcalib3->cd(2);hcalib_PID3[1]->Draw("colz");
  cPIDcalib3->Modified();cPIDcalib3->Update();cPIDcalib3->Write();

  // ====== F7 GeE ================================================================
  TCanvas *cGeE = new TCanvas("cGeE","cGeE",1200,1200*0.7);
  cGeE->Divide(2,4);
  // cGeE->cd(1);hcalib_Ge_E[0]->Draw("colz");
  // cGeE->cd(2);hcalib_Ge_E[1]->Draw("colz");
  // cGeE->cd(3);hcalib_Ge_Esum[0]->Draw("colz");
  // cGeE->cd(4);hcalib_Ge2_Esum[1]->Draw("colz");
  cGeE->SetLogy(1);
  for(int ii=0;ii<8;ii++){
    cGeE->cd(ii+1);hcalib_F7Ge_ESpec[ii]->Draw("");
    cGeE->Modified();cGeE->Update();
  }
  cGeE->Write();
  // ====== F7 GeET ================================================================
  TCanvas *cGeET = new TCanvas("cGeET","cGeET",1200,1200*0.7);
  cGeET->Divide(4,4);
  for (int ii = 0; ii < 16; ii++) {
    cGeET->cd(ii+1);hcalib_Ge_EvsT[ii]->Draw("colz");
  }
