...
#include "signal.h"
#include "./include/ribf_interrupt.h"

#include "./include/vme/ribf_var.h"
#include "./include/vme/ribf_para.h" 
#include "./include/vme/ribf_validgate.h" // only for unpacking rawdata
#include "./include/vme/ribf_raw.h"       // only for unpacking rawdata
#include "./include/vme/ribf_branch.h"
#include "./include/vme/ribf_encpid.h"
#include "./include/ribf_transmission.h"
...
void VME2Tree(int runN=48, TString runname="56Co", bool tree_output=0 /* 0 = NO, 1 = Yes */){
  ...
  */
  ...
  ifname = "./ridf/vme/"      + runname + Form("%04d.ridf",runN);

  if(tree_output){
    //    ofname = "./hist/vme/tree/" + runname + Form("%04d.root",runN);
    ofname = "./rootfiles/" + runname + Form("%04d.root",runN);
  }else{
    ofname = "./hist/vme/hist/" + runname + Form("%04d.root",runN);
  }

  if(runN==9999)ofname="./hist/vme/temp.root";
...
#ifdef ONLINE_FLAG
  // defined in include/vme/ribf_branch.h
  ribf_branch(tree); 
#else
  // defined in include/vme/ribf_branch.h
  ...
  */
  ...
  //    while(estore->GetNextEvent()&&neve<10000){
	...
	*/
    ...
    // defined in include/vme/ribf_raw.h
    rawdata_reset();
#ifdef ONLINE_FLAG
    // defined in include/vme/ribf_encpid.h
    var_init();
#endif
    //===== Segment Loop ==============================
