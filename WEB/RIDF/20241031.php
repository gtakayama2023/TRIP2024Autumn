...
if (($handle = fopen($csvFile, 'r')) !== false) {
	...
	if (strpos($currentUrl, 'ridf.php') !== false) {
	    // ridf.phpの場合、「10k」ボタンのみを表示
	    echo "<button class='exe-button' type='button' onclick=\"window.open('/$base_url/cgi-bin/EXP/RIBF/TRIP/2024/AUTUMN/USR/default/WEB/ridf2hist?runNo=$runNumber&runName=" . urlencode($runName) . "&loadEvts=" . $loadEvts['10k'] . "', '_blank')\">10k</button> ";
	} elseif (strpos($currentUrl, 'ridf.auth.php') !== false) {
	    // ridf.auth.phpの場合、全ボタンを表示
	    foreach ($loadEvts as $label => $value) {
	        echo "<button class='exe-button' type='button' onclick=\"window.open('/$base_url/cgi-bin/EXP/RIBF/TRIP/2024/AUTUMN/USR/default/WEB/ridf2hist?runNo=$runNumber&runName=" . urlencode($runName) . "&loadEvts=$value', '_blank')\">$label</button> ";
	    }
	}
	echo "</td>";
