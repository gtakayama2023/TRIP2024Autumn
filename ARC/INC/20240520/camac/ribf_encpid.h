...
void ribf_encpid(int runN,TRandom *grand){
  ...
  for(int l=1;l<12;l++){
	...
	//===== Fired or not ==========
	for(int n=0;n<4;n++){
	  if(0<PPAC_Traw[l][m][n]&&PPAC_Traw[l][m][n]<4090){PPAC_fT[l][m][n]=true;}else{PPAC_fT[l][m][n]=false;}
	}
	if(0<PPAC_ATraw[l][m]&&PPAC_ATraw[l][m]<4090){PPAC_fAT[l][m]=true;}else{PPAC_fAT[l][m]=false;}
 ...
 // F9-F11
  ...
  for(int m=0;m<3;m++){
    ...
    double de_v;
    de_v 
      = TMath::Log(ionpair[m] * pow(temp_Beta,2.)) - TMath::Log(1. - pow(temp_Beta,2.)) - pow(temp_Beta,2.);

    double temp_zet;
    temp_zet = Zcoef[m][0] * sqrt(temp_dE/de_v) * temp_Beta + Zcoef[m][1];

    //    temp_zet = sqrt(temp_dE/(Zcoef[m][0] * temp_TOF + Zcoef[m][1]))*20.;
    switch(m){
