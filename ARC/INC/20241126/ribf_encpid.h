void var_init(){
  ...
  // F7Ge
  for (int mm = 0; mm < 8; mm++) F7Ge_E[mm]     = -999.;
  for (int mm = 0; mm < 2; mm++) F7Ge_Esum[mm]  = -999.;
  for (int mm = 0; mm < 8; mm++) F7Ge2_E[mm]    = -999.;
  for (int mm = 0; mm < 2; mm++) F7Ge2_Esum[mm] = -999.;
  for (int mm = 0; mm < 17; mm++) for (int nn = 0; nn < 5; nn++) F7Ge_T[mm][nn] = -999.;
...
void ribf_encpid(int runN,TRandom *grand){
  ...
  for(int l=0; l<12; l++){
	...
	if (Slew_PLA[0][l][m] != 0) PLA_S[l][m] = PLA_T[l][m] - ( Slew_PLA[0][l][m] * pow(PLA_Q[l][m]+Slew_PLA[1][l][m],-0.5) + Slew_PLA[2][l][m] );
	if (Slew_PLA[0][l][m] == 0) PLA_S[l][m] = PLA_T[l][m];
	PLA_QTC[l][m] = ch2MeV_PLA_QTC[0][l][m] * ( (double)PLA_QTCraw[l][m] + grand->Uniform(0,1) - ch2MeV_PLA_QTC[1][l][m] );
      ...
      //PLA_Tave[l] = 0.5 * ( PLA_S[l][0] + PLA_S[l][1] ); // Temporary 2024/11/03
  ...
  for(int mm = 0; mm < 8; mm++) {
    F7Ge_E[mm] = ch2keV_F7Ge[mm][0] * (F7Ge_Eraw[mm] + grand->Uniform(0, 1) + ch2keV_F7Ge[mm][1]);
    F7Ge2_E[mm] = ch2keV_F7Ge[mm][0] * (F7Ge_Eraw[mm] + grand->Uniform(0, 1) + ch2keV_F7Ge[mm][1]);
  }
  F7Ge_Esum[0] = F7Ge_E[0] + F7Ge_E[1] + F7Ge_E[2] + F7Ge_E[3]; // L
  F7Ge_Esum[1] = F7Ge_E[4] + F7Ge_E[5] + F7Ge_E[6] + F7Ge_E[7]; // R
  for (int mm = 0; mm < 32; mm++) {
    for (int nn = 0; nn <N_Mhit; nn++) {
      F7Ge_T[mm][nn] = ch2ns_F7Ge[mm] * F7Ge_Traw[mm][nn];
    }
  }
